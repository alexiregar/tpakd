...
<script type='text/javascript' src='https://platform-api.sharethis.com/js/sharethis.js#property=61c876229ebb4b00195d6642&product=sop' async='async'></script>
		...
		<div class="container">
			<div class="row">
				<div class="col-xs-12 text-center">
					...
					Copyright &copy; 2021 Website Resmi <?php echo $tiden['nama_pemilik']; ?> All Right Reserved.</a>
					</p>
...
<script>
	$(document).on('ready', ()=> {
		let loading = '<center><img src="https://www.catf.us/wp-content/themes/catf-5ive/assets/dist/img/progress.gif" width="15px" alt=""> Loading</center>';
		if($('#dkomoditas').length > 0){
			$('#dkomoditas').load('harga_pasar_load.php',(data) => {
				btnCek();
			});
		}

		$('#komoditasAct').on('change', function(){
			$('#dkomoditas').html(loading);

			let id = $('#komoditasAct').find(':selected').val();
			$('#dkomoditas').load('harga_pasar_load.php?id=' + id, (data) => {
				btnCek();
			});
		});

		function btnCek(){
			$('.checkgrafik').on('click', function(){
				let id_pasar = $(this).data('idpasar');
				let id_komoditas = $(this).data('idkomoditas');

				$('#dkomoditas').html(loading);

				$('#dkomoditas').load('harga_pasar_load.php?page=komoditas&id_pasar=' + id_pasar + '&id_komoditas=' + id_komoditas, (data) => {
					btnBack();
				});
			});
		}
